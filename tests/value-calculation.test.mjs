...
test('partial priced usage retains an approximate partial subscription value',async t=>{
 ...
 const v=enrichSnapshot(snapshot,s,{plans:new Map()},{subscriptionFor:()=>({monthlyUsd:200}),pricingSources:[]},now).providers[0].accounts[0];
...
test('absent account usage has no monthly value basis and unattributed costs make its ratio partial',async t=>{
 ...
 const evaluate=()=>enrichSnapshot(snapshot,s,{plans:new Map()},{subscriptionFor:()=>({monthlyUsd:200}),pricingSources:[]},now).providers[0].accounts[0].analytics;
...
test('five-hour recommendation includes the first positive priced request in a short history',async t=>{
 ...
 const r=enrichSnapshot(snapshot,s,{plans:new Map()},{subscriptionFor:()=>({monthlyUsd:200}),pricingSources:[]},now).providers[0].analytics.recommendation;
 assert.equal(r.peakFiveHourAccounts,3);assert.equal(r.minimumAccounts,3);assert.equal(r.recommendedAccounts,4);
});
...
test('each period tells an unpriced call apart from an amount that really is zero',async t=>{
 ...
 const snapshot={providers:[{id:'cursor',accounts:[{id:'a',status:'ok',plan:'ultra',windows:[]}]}]};
 const periods=enrichSnapshot(snapshot,s,{plans:new Map()},{subscriptionFor:()=>({monthlyUsd:200}),pricingSources:[]},now).providers[0].accounts[0].analytics.periods;
 ...
 const none=enrichSnapshot(snapshot,quiet,{plans:new Map()},{subscriptionFor:()=>({monthlyUsd:200}),pricingSources:[]},now).providers[0].accounts[0].analytics.periods;
