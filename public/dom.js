import {finite, percent, date} from './format.js';
...
export function quotaTrack(remaining, label) {
  ...
  if (finite(remaining)) {
    ...
    track.setAttribute('aria-valuetext', `${percent.format(remaining)}% 남음`);
    const fill = node('div', 'fill'); fill.style.width = `${remaining}%`; track.append(fill);
  }
  return track;
}
export function historyChart(history, label) {
  const wrap = node('div', 'spark');
  const points = Array.isArray(history) ? history.filter(p => p && finite(Date.parse(p.at)) && finite(p.usedPercent)) : [];
  if (points.length < 2) {
  ...
  chart.setAttribute('aria-label', `${label} 사용률 추이, ${points.length}개 측정`);
  const title = svg('title'); title.textContent = `${label} 사용률 추이`; chart.append(title);
  let path = '', lastReset = points[0].resetAt ?? '';
  for (let i = 0; i < points.length; i++) {
    const x = pad + (times[i] - minT) / span * (w - pad * 2);
    const y = pad + (1 - Math.min(100, Math.max(0, points[i].usedPercent)) / 100) * (h - pad * 2);
    const reset = points[i].resetAt ?? '';
    if (i === 0 || (reset !== lastReset && Math.abs(Date.parse(reset)-Date.parse(lastReset)) > 60000) || (i > 0 && (points[i].usedPercent < points[i-1].usedPercent || times[i]-times[i-1] > 20*60000))) {
      if (path) {
  ...
  if (path) chart.append(svg('path', {d:path}));
  figure.append(chart);
  wrap.append(figure);
  return wrap;
}
export function sourceLink(source) {
  ...
  if (source?.url) {
    ...
    a.target = '_blank';
    item.append(a);
    if (source.checkedAt) item.append(document.createTextNode(` · ${date.format(new Date(source.checkedAt))}`));
  } else item.textContent = source?.label || '출처 확인 중';
  return item;
}
export function methodology(analytics, extra) {
  ...
  if (extra?.body) box.append(extra.body);
  if (!notes.length && !sources.length && !extra?.body) box.append(node('p', 'unavailable', '출처가 아직 없습니다.'));
  return box;
}
