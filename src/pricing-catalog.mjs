...
import { priceUsage, lookupModelPrice } from './pricing.mjs';
...
export async function createPricingCatalog({file = process.env.QUOTA_MODEL_CATALOG || join(process.env.XDG_CACHE_HOME || join(homedir(),'.cache'),'opencode','models.json'), now = Date.now} = {}) {
 ...
 const source = await Promise.all(['pricing.mjs','opencode-pricing.mjs','pricing-catalog.mjs','cursor-cache.mjs'].map(name=>readFile(new URL(name,import.meta.url))));
 ...
 const price = row => claudeCache(row, value => priceCursorCache(value, v => priceUsage(v,{catalog:lookup})), lookup);
